...
Once you have setup Stack Trace Linking, Sentry can use information from your source code provider to suggest the commit that likely introduced the error. For example, after pinpointing the suspect commit, you can also identify the developer who made the commit and assign them the task of fixing the error.

![Issue detail highlighting suspect commits](highlighting-suspect-commits.png)

Here is where you can find info for [suspect commit setup](/product/issues/suspect-commits/).
